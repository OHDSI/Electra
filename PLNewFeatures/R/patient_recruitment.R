...
#' the cohort per time period. The function also saves the results to PostgreSQL database.
#'
...
patient_recruitment <- function(cohort_id,
  ...
  start_date <- data.table::as.data.table(
    DBI::dbGetQuery(conn,
  ...
  to_plot <- save_to_postgres(
    ...
    function_name = "patient_recruitment"
  )
